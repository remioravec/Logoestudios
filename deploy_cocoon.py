...
ORTHO_TROUBLES = {
    ...
    "dysphasie": {
        ...
        "approach": "La prise en charge de la dysphasie chez Logopsi Études combine stimulation langagière intensive, supports visuels type pictogrammes et renforcement de la syntaxe par des exercices structurés. La régularité des séances en ligne facilite un suivi soutenu essentiel pour ce trouble.",
        "faq": [
...
def generate_pillar_content(discipline: str, troubles: dict, slug_prefix: str) -> str:
    ...
    if discipline == "Orthophonie":
        pro = "orthophoniste"
        intro = (
            "L'orthophonie en ligne chez Logopsi Études, c'est un accès rapide à des "
        ...
        pro = "psychologue"
        intro = (
            "La psychologie en ligne chez Logopsi Études offre un accompagnement psychologique "
...
def deploy(dry_run: bool = False) -> dict:
    ...
    ortho_pillar_id = create_wp_page(
        ...
        excerpt="Orthophonie en ligne pour enfants : bilan et rééducation avec des orthophonistes diplômés. Séances en visioconférence, sans liste d'attente. Logopsi Études.",
        dry_run=dry_run,
    )
    ...
    psy_pillar_id = create_wp_page(
        ...
        content=psy_content,
        excerpt="Psychologue en ligne pour enfants et adolescents : accompagnement psychologique adapté en visioconférence. Professionnels diplômés, sans liste d'attente. Logopsi Études.",
        dry_run=dry_run,
    )
    ...
    for city in CITIES:
        ...
        page_id = create_wp_page(
            ...
            slug=slug,
            content=content,
            excerpt=f"Orthophoniste à {city} en ligne : bilan et rééducation orthophonique en visioconférence depuis {city}. Sans liste d'attente. Logopsi Études.",
    ...
    for city in CITIES:
        ...
        page_id = create_wp_page(
            ...
            slug=slug,
            content=content,
            excerpt=f"Psychologue enfant à {city} en ligne : accompagnement psychologique en visioconférence depuis {city}. Professionnels diplômés. Logopsi Études.",
    ...
    for key, data in ORTHO_TROUBLES.items():
        ...
        page_id = create_wp_page(
            ...
            slug=slug,
            content=content,
            excerpt=f"Orthophoniste spécialisé en {data['full_name']} en ligne. {data['definition'][:100]} Séances en visioconférence chez Logopsi Études.",
            parent=ortho_pillar_id or 0,
            dry_run=dry_run,
        )
        mapping[slug] = page_id
    ...
    for key, data in PSY_TROUBLES.items():
        ...
        page_id = create_wp_page(
            ...
            slug=slug,
            content=content,
            excerpt=f"Psychologue spécialisé en {data['full_name']} en ligne pour enfants. {data['definition'][:100]} Séances en visioconférence chez Logopsi Études.",
            parent=psy_pillar_id or 0,
            dry_run=dry_run,
        )
        mapping[slug] = page_id
    ...
    for key, data in ORTHO_TROUBLES.items():
        for city in CITIES:
            ...
            page_id = create_wp_page(
                ...
                slug=slug,
                content=content,
                excerpt=f"Orthophoniste spécialisé {data['name']} à {city} en ligne. Bilan et rééducation en visioconférence. Logopsi Études.",
    ...
    for key, data in PSY_TROUBLES.items():
        for city in CITIES:
            ...
            page_id = create_wp_page(
                ...
                slug=slug,
                content=content,
                excerpt=f"Psychologue spécialisé {data['name']} à {city} en ligne pour enfants. Séances en visioconférence. Logopsi Études.",
